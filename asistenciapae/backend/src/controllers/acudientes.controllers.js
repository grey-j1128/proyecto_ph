...
export const getAcudiente = async (idAcudiente) => {
  ...
  if (result.length === 0)
    throw {
      name: "Custom error",
      statusCode: 404,
      message: `Acudiente con ID ${idAcudiente} no encontrado.`,
    };
  return result;
};

export const insertAcudiente = async (
  TipoDoc_Acudiente,
  ...
  try {
    ...
    if (error.message.search("uk_acudiente") > -1) {
      throw {
        name: "Custom error",
        statusCode: 409,
        message: `Acudiente con Documento ${TipoDoc_Acudiente}-${Numero_Doc_Acudiente} ya existe.`,
      };
    }
    if (error.message.search("ref_acudientes_to_parentesco") > -1) {
      throw {
        name: "Custom error",
        statusCode: 422,
        message: `El id_Parentesco=${id_Parentesco} no existe.`,
      };
    }
    throw error;
  }
