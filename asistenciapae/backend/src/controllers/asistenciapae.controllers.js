...
export const recibirReportePAE = async (id_estudiante) => {
  ...
  if (result.length === 0)
    throw new DatabaseError(
      404,
      `Estudiante con numero de documento ${numerodoc_estudiante} no agregado al sistema de asistencia PAE`
    );
  return result[0];
};
...
export const validarAlmuerzoHoyEstudiantePAE = async (id_estudiante) => {
  ...
  if (result.length > 0)
    throw new DatabaseError(
      401,
      `Estudiante con número de documento ${result[0].numerodoc_estudiante} ya fue registrado hoy`
    );
  return true;
};
...
export const insertAsistenciaPAE = async (id_estudiante) => {
  ...
  try {
    ...
    if (error.message.search("ref_asistencia_pae_to_estudiantes") > -1) {
      throw new DatabaseError(
        422,
        `El id_estudiante = ${id_estudiante} no existe.`
      );
    }
    throw error;
  }
...
export const updateAcudiente = async (
  idAcudiente,
  TipoDoc_Acudiente,
  Numero_Doc_Acudiente,
  Nombre_Completo,
  Direccion,
  Telefono,
  Email,
  ...
  try {
    ...
    if (result.length === 0)
      throw new DatabaseError(
        404,
        `Acudiente con ID ${idAcudiente} no encontrado.`
      );
    return result;
  } catch (error) {
    if (error.message.search("uk_acudiente") > -1) {
      throw new DatabaseError(
        409,
        `Acudiente con Documento ${TipoDoc_Acudiente}-${Numero_Doc_Acudiente} ya existe.`
      );
    }
    if (error.message.search("ref_acudientes_to_parentesco") > -1) {
      throw new DatabaseError(
        422,
        `El id_Parentesco=${id_Parentesco} no existe.`
      );
    }
    throw error;
  }
