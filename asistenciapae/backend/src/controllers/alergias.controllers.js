...
export const getAlergias = async () => {
  const query =
    "SELECT id_alergia, Nombre_alergia FROM alergias ORDER BY id_alergia ASC";
  try {
...
export const getAlergia = async (idAlergia) => {
  const query =
    "SELECT id_alergia, Nombre_alergia FROM alergias WHERE id_alergia = $1";
  try {
    return await executeQuery(query, [idAlergia]);
  } catch (error) {
    throw new Error(
      `Error al ejecutar consulta de alergia con idAlergia=${idAlergia}: ${error.stack}`
    );
  }
};
...
export const insertAlergia = async (NombreAlergia) => {
  const query =
    "INSERT INTO alergias (nombre_alergia) VALUES ($1) RETURNING id_alergia;";
  try {
    return await executeQuery(query, [NombreAlergia]);
  } catch (error) {
    throw new Error(
      `Error al ejecutar inserción de alergia con NombreAlergia=${NombreAlergia}: ${error.stack}`
    );
  }
};
...
export const updateAlergia = async (idAlergia, NombreAlergia) => {
  const query =
    "UPDATE alergias SET nombre_alergia = $2 WHERE id_alergia = $1 RETURNING *;";
  try {
    return await executeQuery(query, [idAlergia, NombreAlergia]);
  } catch (error) {
    throw new Error(
      `Error al ejecutar actualización de alergia con idAlergia=${idAlergia}: ${error.stack}`
    );
  }
};
...
export const deleteAlergia = async (idAlergia) => {
  ...
  try {
    ...
    throw new Error(
      `Error al ejecutar borrado de alergia con idAlergia=${idAlergia}: ${error.stack}`
    );
  }
};
